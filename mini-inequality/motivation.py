...
OUT02_CSV_COLS = ["project", "sentences", "components_model", "components_gold",
                  "files", "links", "median", "max", "gini", "top3_pct"]
...
def write_out02_concentration():
    ...
    with open(REPORTS / "out02_concentration.csv", "w", newline="") as f:
        ...
        for r in rows:
            w.writerow([FULL_NAMES.get(r["project"], r["project"]),
                        r["sentences"], r["comp_model"], r["comp_n"], r["files"],
    ...
    L = [
        ...
        "% benchmark SAD text, the PCM (SAM) repository, the .acm code model, and",
        ...
        "% gold-reachable components the size-aware suite scores; Files = code",
        "% compilation units in the .acm model; Links/Med/Max/Gini/Top-3\\% = enrolled",
        "% doc-to-code link concentration over the K components.",
        ...
        "\\caption{The five ardoco-benchmark projects: dataset size and the "
        "concentration of their enrolled doc-to-code links across architecture "
        "components. Sent.\\ is the number of sentences in the architecture "
        "documentation, Comp.\\ the number of components in the architecture model, and "
        "Files the number of code files in the code model. $K$ is the number of "
        "components that carry gold doc-to-code links (the universe the size-aware "
        "suite scores) and Links the enrolled link total; the links are heavily "
        "right-skewed across the $K$ components---a few large ones own most of them, so "
        "link-level \\fone\\ mainly reflects those few: Med, Max, Gini, and Top-3\\% "
        "summarize links per component. A target file shared by several components is "
        "counted under each, so per-component counts can sum above Links.}",
        "\\label{tab:gold_concentration}",
        "\\begin{tabular}{lrrrrrrrrr}", "\\toprule",
        "\\textbf{Project} & \\textbf{Sent.} & \\textbf{Comp.} & \\textbf{$K$} & "
        "\\textbf{Files} & \\textbf{Links} & \\textbf{Med} & \\textbf{Max} & "
    ...
    for r in rows:
        L.append(" & ".join([
            ...
            tex_sep(r["files"]), tex_sep(r["links_total"]), tex_sep(r["link_median"]),
