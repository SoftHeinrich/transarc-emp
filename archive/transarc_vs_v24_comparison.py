#!/usr/bin/env python3
"""
TransArc vs V24 Comprehensive Comparison

Compares TransArc (traditional transitive pipeline) against V24 (LLM linker
...
V24 is a SAD-SAM linker; its SAD-CODE result is composed via gold SAM-CODE.

Outputs: TRANSARC_VS_V24_COMPARISON.md
"""
...
OUTPUT_MD = Path("/mnt/hostshare/ardoco-home/transarc-emp/TRANSARC_VS_V24_COMPARISON.md")

V24_DIR = Path("/mnt/hostshare/ardoco-home/llm-sad-sam-v45/results/ablation_results")


# ─── V24 data loaders ───────────────────────────────────────────────────────

def load_v24_sad_sam(project):
    """Load V24 SAD-SAM links as set of (modelElementID, sentence_str)."""
    path = V24_DIR / f"v24_{project}_links.csv"
    links = set()
    if not path.exists():
        return links
    with open(path) as f:
...
def load_v24_sad_sam_with_confidence(project):
    """Load V24 links with confidence for MAP computation."""
    path = V24_DIR / f"v24_{project}_links.csv"
    links = []
    if not path.exists():
        return links
...
def main():
    ...
    w("# TransArc vs V24: Comprehensive Metric Comparison")
    w("")
    ...
    w("- **V24**: LLM linker with hybrid batch/union judge (SAD-SAM), projected through **gold SAM-CODE** for SAD-CODE")
    w("")
    w("V24 is a SAD-SAM linker; its SAD-CODE result is composed transitively via gold SAM-CODE.")
    ...
    for proj in PROJECTS:
        ...
        # ─── V24─────────────────────────────────────────────────────
        v24_sad_sam = load_v24_sad_sam(proj)
        v24_ranked = load_v24_sad_sam_with_confidence(proj)
        v24_sad_code = compose_sad_code(v24_sad_sam, gs_sam_code_map)
        ...
        v_p, v_r, v_f1, v_tp, v_fp, v_fn = calc_metrics(gs_sad_sam, v24_sad_sam)

        tc_p, tc_r, tc_f1, tc_tp, tc_fp, tc_fn = calc_metrics(gs_sad_code, transarc_sad_code)
        vc_p, vc_r, vc_f1, vc_tp, vc_fp, vc_fn = calc_metrics(gs_sad_code, v24_sad_code)
        ...
        w(f"| SAD-SAM | V24| {v_p:.3f} | {v_r:.3f} | {v_f1:.3f} | {v_tp} | {v_fp} | {v_fn} |")
        w(f"| SAD-SAM | **Δ (V24−T)** | {v_p-t_p:+.3f} | {v_r-t_r:+.3f} | **{v_f1-t_f1:+.3f}** | {v_tp-t_tp:+d} | {v_fp-t_fp:+d} | {v_fn-t_fn:+d} |")
        w(f"| SAD-CODE | TransArc | {tc_p:.3f} | {tc_r:.3f} | {tc_f1:.3f} | {tc_tp} | {tc_fp} | {tc_fn} |")
        w(f"| SAD-CODE | V24| {vc_p:.3f} | {vc_r:.3f} | {vc_f1:.3f} | {vc_tp} | {vc_fp} | {vc_fn} |")
        w(f"| SAD-CODE | **Δ (V24−T)** | {vc_p-tc_p:+.3f} | {vc_r-tc_r:+.3f} | **{vc_f1-tc_f1:+.3f}** | {vc_tp-tc_tp:+d} | {vc_fp-tc_fp:+d} | {vc_fn-tc_fn:+d} |")
        w("")
        ...
        n1_v = compute_mcc(gs_sad_sam, v24_sad_sam, all_sents, all_comps)

        n1c_t = compute_mcc(gs_sad_code, transarc_sad_code, all_sents, all_code_files)
        n1c_v = compute_mcc(gs_sad_code, v24_sad_code, all_sents, all_code_files)
        ...
        w(f"| SAD-SAM | V24| {n1_v['mcc']:.3f} | {n1_v['balanced_accuracy']:.3f} |")
        ...
        w(f"| SAD-CODE | V24| {n1c_v['mcc']:.3f} | {n1c_v['balanced_accuracy']:.3f} |")
        ...
        for level, label, data in [
            ("SAD-SAM", "TransArc", n2_t), ("SAD-SAM", "V24", n2_v),
            ("SAD-CODE", "TransArc", n2c_t), ("SAD-CODE", "V24", n2c_v),
        ]:
        ...
        n4_v = compute_acf1(gs_sad_code, v24_sad_code, gs_sam_code_map)
        ...
        w(f"| V24| {vc_f1:.3f} | {n4_v['acf1']:.3f} | {n4_v['acf1']-vc_f1:+.3f} |")
        w(f"| **Δ (V24−T)** | **{vc_f1-tc_f1:+.3f}** | **{n4_v['acf1']-n4_t['acf1']:+.3f}** | |")
        w("")
        ...
        n6_v = compute_hus(gs_sad_sam, v24_sad_sam)

        n6c_t = compute_hus(gs_sad_code, transarc_sad_code)
        n6c_v = compute_hus(gs_sad_code, v24_sad_code)
        ...
        w(f"| SAD-SAM | V24| {n6_v['hus']:.3f} | {n6_v['coverage']:.3f} | {n6_v['purity']:.3f} |")
        w(f"| SAD-CODE | TransArc | {n6c_t['hus']:.3f} | {n6c_t['coverage']:.3f} | {n6c_t['purity']:.3f} |")
        w(f"| SAD-CODE | V24| {n6c_v['hus']:.3f} | {n6c_v['coverage']:.3f} | {n6c_v['purity']:.3f} |")
        w("")
        ...
        idf_v = compute_idf_weighted_f1(gs_sad_code, v24_sad_code, code_model)
        macro_t = compute_component_macro_f1(gs_sad_code, transarc_sad_code, gs_sam_code_map)
        macro_v = compute_component_macro_f1(gs_sad_code, v24_sad_code, gs_sam_code_map)
        pdr_t = compute_pdr_f1(gs_sad_code, transarc_sad_code, gs_sam_code_map)
        pdr_v = compute_pdr_f1(gs_sad_code, v24_sad_code, gs_sam_code_map)
    ...
    w("### SAD-SAM F1")
    w("")
    w("| Project | TransArc | V24| Δ | Winner |")
    w("|:--|:---:|:---:|:---:|:--|")
    for i, proj in enumerate(PROJECTS):
        tf = agg["std_sam"]["t"][i]
        vf = agg["std_sam"]["v"][i]
        delta = vf - tf
        winner = "V24" if delta > 0.005 else ("TransArc" if delta < -0.005 else "Tie")
        w(f"| {proj} | {tf:.3f} | {vf:.3f} | {delta:+.3f} | {winner} |")
    avg_t = sum(agg["std_sam"]["t"]) / len(PROJECTS)
    avg_v = sum(agg["std_sam"]["v"]) / len(PROJECTS)
    w(f"| **Average** | **{avg_t:.3f}** | **{avg_v:.3f}** | **{avg_v-avg_t:+.3f}** | {'**V24**' if avg_v > avg_t else '**TransArc**'} |")
    w("")
    ...
    w("")
    w("| Metric | Level | TransArc | V24| Δ | Winner |")
    w("|:--|:--|:---:|:---:|:---:|:--|")
    ...
    w(f"**Score: V24wins {wins['v']}, TransArc wins {wins['t']}, Ties {wins['tie']}** "
    ...
    w("")
    w("| Project | TransArc Wins | V24Wins | Ties |")
    w("|:--|:---:|:---:|:---:|")
    ...
    w(f"1. **SAD-SAM**: V24avg F1 = {sam_avg_v:.3f} vs TransArc {sam_avg_t:.3f} "
      f"(Δ={sam_avg_v-sam_avg_t:+.3f})")
    w(f"2. **SAD-CODE** (standard): V24avg F1 = {code_avg_v:.3f} vs TransArc {code_avg_t:.3f} "
      f"(Δ={code_avg_v-code_avg_t:+.3f})")
    w(f"3. **SAD-CODE** (debiased macro): V24= {macro_avg_v:.3f} vs TransArc {macro_avg_t:.3f} "
      f"(Δ={macro_avg_v-macro_avg_t:+.3f})")
    w(f"4. **MCC SAD-SAM**: V24= {mcc_sam_v:.3f} vs TransArc {mcc_sam_t:.3f} "
    ...
    w(f"**SAD-SAM project wins**: V24{sam_wins['v']}/{len(PROJECTS)}, "
      f"TransArc {sam_wins['t']}/{len(PROJECTS)}")
    w(f"**SAD-CODE project wins**: V24{code_wins['v']}/{len(PROJECTS)}, "
    ...
    w("python3 transarc_vs_v24_comparison.py")
    w("```")
    w("")
