...
| 2026-06-25 | rq12-bigtable-csv | New `mini-src/rq12.py` (stdlib, imports `metrics.py`; no new metric math) sweeps the 4-system roster, macro-averages 5 projects, means the approach over 3 runs, and emits one wide CSV `reports/RQ12_BIGTABLE.csv` covering every RQ1/RQ2 cell. RQ1 reproduces the paper exactly (3 dp, all systems × both tasks); RQ2 SOTA tail (TransArc .54/.67/.75, Artemis .35/.47/.71) reproduces. `check.py` PASS. Known gap: RQ2 approach tail can't reconcile to paper .62/.71 — source run set `v2.6.5_s20union_gpt_re_medium` + `/tmp/v265.py` deleted; surviving data gives worst .59 / harmonic .78 (file-F1 still .875). Map: RQ1-ss=`ss_P/R/linkF1`, RQ1-sc=`sc_P/R/fileF1`, RQ2=GPT-5.4 rows' `sc_fileF1/sentCov/worstC/harmC` + Δ row. See `.planning/quick/20260625-rq12-bigtable-csv/SUMMARY.md`. |
